...
knitr::opts_chunk$set(results = "hide", cache = TRUE, cache.path = "/vignettes/_cache/", message = FALSE, warning=FALSE, eval=TRUE, echo=FALSE, error=FALSE, fig.align='center',
                      out.width = "85%")
```
...
```{r}
devtools::install_github("trashbirdecology/regimedetectionmeasures", force=TRUE, dependencies = TRUE)
...
```{r, echo=TRUE}
munge_orig_dat(example = T, fill=NA) %>% glimpse()
```
...
We can visualize these metrics:
```{r}
...
[1] Spanbauer, Trisha L., Craig R. Allen, David G. Angeler, Tarsha Eason, Sherilyn C. Fritz, Ahjond S. Garmestani, Kirsty L. Nash, and Jeffery R. Stone. "Prolonged instability prior to a regime shift." PLoS One 9, no. 10 (2014): e108936.
