...
#' @param data A data frame with columns: sortVar: usually time or some spatial dimension; variable: usually species; value: observations about the variable (e.g., count)
#' @param fill Used in the function _calc_vi_. Fill value for missing data. Default = 0
...
#' @export
rdm_window_analysis <- function(data,
                                winMove = 0.25,
                                min.window.dat = 2,
                                fi.equation = '7.12',
                                to.calc = c('VI', 'FI', 'EWS'),
  ...
  ## to throw an error if multiple sites exist
  if ("site" %in% names(data) &
      n_distinct(data$site) > 1) {
    warning("Please provide a data frame with a single unique `site`.")
  }

  sortVar <- data %>% distinct(sortVar) %>% arrange(sortVar)
  ...
  winStart <- round(seq(min(data$sortVar), max(data$sortVar) -
  ...
  # Loop
  for (i in 1:nWin) {
    winData <- data %>% filter(sortVar >= winStart[i],
                               sortVar < winStop[i]) %>% distinct()

    if (length(unique(winData$sortVar)) < min.window.dat |
        nrow(winData) <=
        min.window.dat) {
      warning(
        "# time points < min.window.dat time points -- need more to calculate metrics.
    ...
    if ("FI" %in% to.calc) {
      FI_temp = NULL
      FI_temp <- calc_fi(dataInFI = winData,
    ...
    if ("VI" %in% to.calc) {
      VI_temp = NULL
      VI_temp <- calc_vi(winData, fill = fill) %>%
  ...
  resultsOut <- do.call(plyr::rbind.fill,
                        lapply(resultsOut,
